# Over 5 + Under 4 — fréquences sur toutes les volatilités
...
- Un abonnement par indice avec historique de 200 ticks ; précision fournie par l'API. Les historiques restent séparés, ordonnés et dédupliqués par timestamp. Un flux de plus de 5 secondes n'est pas sélectionné.
- Après au moins 100 ticks : fréquences 6–9 et 0–3 d'au moins 30 % chacune, total d'au moins 82 %, et couverture 0–3/6–9 d'au moins 80 % sur les 50 derniers ticks.
...
L'espérance estimée de la paire est `p_over × payout_over + p_under × payout_under − coût_total`. Le versement comprend la mise. La paire n'est envoyée que si cette espérance estimée dépasse 0,5 % de son coût. Ce calcul repose sur des fréquences historiques lissées non calibrées ; il ne démontre pas une rentabilité future.
...
La mise est fixe pour les deux contrats : martingale, double risque et risque demi-solde ne sont pas appliqués à cette stratégie. Aucune pause de huit ticks n'est ajoutée ; les indices récemment refusés sont temporairement écartés pour permettre la comparaison des suivants.
