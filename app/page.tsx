...
import { RUN_STRATEGIES, evaluateRunEntry, parseRunStrategyMarkdown, type RunStrategy } from "@/lib/only-ups-downs";
import { Mt5CopyTradingPanel } from "@/components/mt5-copytrading-panel";
import { RunStrategyPanel } from "@/components/run-strategy-panel";
...
import { updateContractProgress, contractPhaseLabel, type ContractProgressUpdate } from "@/lib/deriv-contract-progress";
...
type DerivContractCategory = "rise_fall" | "matches_differs" | "over_under" | "even_odd" | "touch_no_touch" | "only_ups_downs";
type DerivContractCode = "CALL" | "PUT" | "DIGITMATCH" | "DIGITDIFF" | "DIGITOVER" | "DIGITUNDER" | "DIGITEVEN" | "DIGITODD" | "ONETOUCH" | "NOTOUCH" | "RUNHIGH" | "RUNLOW";
...
const derivContractCategories: Record<DerivContractCategory, { name: string; description: string; options: DerivContractCode[] }> = {
  ...
  only_ups_downs: { name: "Only Ups / Only Downs", description: "Only Ups : chaque tick monte strictement après l’entrée. Only Downs : chaque tick descend strictement. Un tick égal ou contraire fait perdre le contrat.", options: ["RUNHIGH", "RUNLOW"] },
...
function parseAdvancedMatchStrategyMarkdown(markdown: string): ImportedMatchStrategy {
  ...
  if (data.executionMode === "dbx_fixed" || data.executionMode === "dbx_dynamic" || data.executionMode === "dbx_last_digit") {
    ...
    const manualDigit = data.executionMode === "dbx_dynamic" && data.barrierMode === "fixed";
    if (manualDigit && (typeof data.fixedDigit !== "number" || !Number.isInteger(data.fixedDigit) || data.fixedDigit < 0 || data.fixedDigit > 9)) {
      throw new Error("DBX V3.1 : le digit manuel doit être un entier de 0 à 9");
    }
    return { ...profile,
      ...(manualDigit ? { barrierMode: "fixed" as const, fixedDigit: data.fixedDigit as number } : {}), stake: clampNumber(data.stake, DBX_MATCH_CONFIG.stake, 0.35, 10_000),
      durationTicks: Math.trunc(clampNumber(data.durationTicks, DBX_MATCH_CONFIG.duration, 1, 10)),
  ...
  return {
    executionMode: "statistical",
    durationTicks: Math.trunc(clampNumber(data.durationTicks, 1, 1, 10)),
    lossBudgetStakes: null,
...
function getContractCategory(contractType: DerivContractCode): DerivContractCategory {
  if (contractType === "RUNHIGH" || contractType === "RUNLOW") return "only_ups_downs";
...
function chooseDerivContractDuration(contractType: DerivContractCode, ticks: number[]): DurationDecision {
  if (contractType === "RUNHIGH" || contractType === "RUNLOW") return { duration: 2, label: "SÉRIE", reason: "Chaque tick doit conserver le sens choisi, sans égalité." };
  if (contractType.startsWith("DIGIT")) {
    return { duration: 1, label: "DIGIT", reason: "Durée du contrat après son entrée confirmée par Deriv" };
  }
...
export default function Home() {
  ...
  const [runStrategy, setRunStrategy] = useState<RunStrategy>(RUN_STRATEGIES.reactive);
  const runStrategyRef = useRef<RunStrategy>(RUN_STRATEGIES.reactive);
  const [runStrategySelection, setRunStrategySelection] = useState("reactive");
  const [importedRunStrategy, setImportedRunStrategy] = useState<RunStrategy | null>(null);
  const [runImportStatus, setRunImportStatus] = useState("");
  const runImportIdRef = useRef(0);
  const [derivRunDuration, setDerivRunDuration] = useState(2);
  const derivRunDurationRef = useRef(2);
  ...
  const mt5ConnectedMode: AccountMode | null = mt5Online && mt5Demo !== null ? (mt5Demo ? "demo" : "real") : null;
	  ...
	  const derivDuration = derivContractType === "DIGITMATCH" ? matchStrategy.durationTicks ?? 1 : derivContractType === "RUNHIGH" || derivContractType === "RUNLOW" ? derivRunDuration : derivDurationDecision.duration;
  ...
  function connectDerivSocket(url: string, mode: "public" | AccountMode) {
    ...
    socket.onmessage = (event) => {
        ...
        if (autoQuote) {
          ...
          if (!derivAutoRunningRef.current || !isDerivTradingStatus(derivStatusRef.current)) {
            ...
            if (autoQuote.dbxGuardRequestedAt !== undefined) {
              ...
              const current = buildMatchPrediction(derivTicksRef.current, derivPipSizeRef.current, active.barrierMode === "fixed" ? active.fixedDigit : null, active.rules).bestCandidate;
              ...
              if (!evaluation.accepted) {
                setDerivAutoStatus(`DBX V3.1 · refus ${active.dbxMinimumProbability == null ? "payout automatique" : "seuil manuel"} · seuil requis ${evaluation.requiredProbability === null ? "indisponible" : (evaluation.requiredProbability * 100).toFixed(2) + "%"} · ${active.dbxMinimumProbability == null ? "estimation prudente" : "estimation du modèle retenue"} ${(evaluation.evaluatedProbability * 100).toFixed(2)}% · équilibre payout ${evaluation.breakEven === null ? "indisponible" : (evaluation.breakEven * 100).toFixed(2) + "%"}`);
                return;
              }
            ...
            if (autoQuote.contractType === "RUNHIGH" || autoQuote.contractType === "RUNLOW") {
              const signal = evaluateRunEntry(derivTicksRef.current, autoQuote.contractType, runStrategyRef.current);
              const age = autoQuote.runRequestedAt === undefined ? Infinity : Date.now() - autoQuote.runRequestedAt;
              if (age < 0 || age > 3000 || derivMarketRef.current !== autoQuote.symbol
                || derivContractTypeRef.current !== autoQuote.contractType || autoQuote.duration !== derivRunDurationRef.current
                || autoQuote.runStrategy !== runStrategyRef.current || !signal.ready
                || !validDbxQuote(proposal.ask_price, proposal.payout ?? 0, autoQuote.stake, derivBalanceRef.current)) {
                setDerivAutoStatus("Only Ups / Only Downs · cotation périmée, signal modifié ou fonds insuffisants · aucun achat");
                return;
              }
            }
      ...
      if (typeof buy?.contract_id === "number") {
        ...
        setDerivDeals((current) => [{ contractId: buy.contract_id!, buyPrice: buy.buy_price ?? pending.stake, payout: buy.payout ?? 0, contractType: pending.contractType, barrier: pending.barrier, symbol: pending.symbol, createdAt: new Date().toISOString(), duration: pending.duration, status: "open" as const, profit: null, entrySpot: null, currentSpot: null }, ...current].slice(0, 24));
        setDerivProposal(null);
        setDerivMessage(`Contrat #${buy.contract_id} acheté · durée demandée : ${pending.duration} tick(s) · attente du tick d’entrée Deriv`);
      ...
      const openContract = message.proposal_open_contract as (ContractProgressUpdate & {
        contract_id?: number;
        buy_price?: string | number;
        payout?: string | number;
      ...
      if (typeof openContract?.contract_id === "number") {
        ...
        setDerivDeals((current) => current.map((deal) => deal.contractId !== openContract.contract_id ? deal : {
          ...
          ...updateContractProgress(deal, openContract),
        ...
        if (isSold) {
          ...
          setDerivMessage(`Contrat #${openContract.contract_id} · ${openContract.is_expired === true || openContract.is_expired === 1 ? "En attente du règlement Deriv" : "Suivi reçu · résultat en attente"}`);
        }
      }
    };
    socket.onerror = () => {
  ...
  function maybeRunDerivAuto(currentTicks: number[], socket: WebSocket) {
    ...
    if (currentContractType === "DIGITMATCH" && isDbxMode(currentMatchStrategy.executionMode)) {
      ...
      const prediction = dynamic ? buildMatchPrediction(currentTicks, derivPipSizeRef.current, guarded && currentMatchStrategy.barrierMode === "fixed" ? currentMatchStrategy.fixedDigit : null, currentMatchStrategy.rules) : null;
      ...
      const order = buildDbxMatchOrder(derivStakeRef.current, digit, currentMatchStrategy.durationTicks ?? DBX_MATCH_CONFIG.duration);
    ...
    const isRunContract = currentContractType === "RUNHIGH" || currentContractType === "RUNLOW";
    ...
    const runSignal = isRunContract ? evaluateRunEntry(currentTicks, currentContractType, runStrategyRef.current) : null;
    if (runSignal && !runSignal.ready) {
      setDerivAutoStatus(runSignal.reason);
      return;
    }
    const selectedContractType = digitSignal?.contractType ?? (needsTouchBarrier(currentContractType) || isRunContract ? currentContractType : priceSignal!.direction);
    const duration = selectedContractType === "DIGITMATCH" ? currentMatchStrategy.durationTicks ?? 1 : isRunContract ? derivRunDurationRef.current : (selectedContractType === "CALL" || selectedContractType === "PUT") && priceSignal
    ...
    const signalReason = digitSignal?.reason ?? (isRunContract ? runSignal!.reason : needsTouchBarrier(selectedContractType) ? `${derivContractLabels[selectedContractType]} dynamique · barrière ${barrier}` : priceSignal!.reason);
    const signalConfidence = digitSignal?.confidence ?? priceSignal?.confidence ?? 0;
    ...
    registerDerivSessionSignal();
    setDerivAutoStatus(`${signalReason}${(isRunContract || isFastMatchMode(currentMatchStrategy.rules.selectionMode) && currentContractType === "DIGITMATCH") ? "" : ` · confiance ${signalConfidence}%`}${fixedBarrierSuffix}${martingaleSuffix} · ${positionTotal} position${positionTotal > 1 ? "s" : ""}`);
  ...
  function requestDerivAutoPosition(socket: WebSocket, input: DerivPendingBuy & { batchIndex: number; batchTotal: number }) {
    ...
    const reqId = ++derivReqIdRef.current;
    derivAutoQuoteRef.current.set(reqId, { ...input, ...(input.contractType === "RUNHIGH" || input.contractType === "RUNLOW" ? { runRequestedAt: Date.now(), runStrategy: runStrategyRef.current } : {}), reqId });
    setDerivAutoStatus(`Cotation en attente · contrat pas encore acheté · position ${input.batchIndex ?? 1}/${input.batchTotal ?? 1} · ${formatDerivContract(input.contractType, input.barrier)}`);
  ...
  function buildDerivProposalRequest(input: { contractType: DerivContractCode; barrier: DerivBarrier; stake: number; currency: string; duration: number; symbol: string; reqId?: number }) {
    if ((input.contractType === "RUNHIGH" || input.contractType === "RUNLOW")
      && (!Number.isInteger(input.duration) || input.duration < 2 || input.duration > 5)) throw new Error("Only Ups / Only Downs : durée de 2 à 5 ticks requise");
    const request: Record<string, string | number> = {
      proposal: 1,
      amount: input.stake,
      basis: "stake",
  ...
  function runStrategyLocked() {
    return derivAutoRunningRef.current || derivOpenContractsRef.current.size > 0
      || derivPendingBuysRef.current.size > 0 || derivAutoQuoteRef.current.size > 0;
  }

  function selectRunStrategy(selection: string, supplied?: RunStrategy) {
    if (runStrategyLocked()) { setRunImportStatus("Arrêtez le bot et attendez la fin des opérations avant de changer de stratégie."); return false; }
    const profile = supplied ?? (selection === "imported" ? importedRunStrategy : Object.hasOwn(RUN_STRATEGIES, selection) ? RUN_STRATEGIES[selection] : null);
    if (!profile) return false;
    runImportIdRef.current++;
    const contractType = profile.contractType ?? (derivContractTypeRef.current === "RUNLOW" ? "RUNLOW" : "RUNHIGH");
    runStrategyRef.current = profile;
    derivRunDurationRef.current = profile.durationTicks;
    derivContractTypeRef.current = contractType;
    derivModeRef.current = "auto";
    setRunStrategy(profile);
    setRunStrategySelection(selection);
    setDerivRunDuration(profile.durationTicks);
    setDerivContractType(contractType);
    setDerivContractCategory("only_ups_downs");
    setDerivMode("auto");
    if (profile.stake !== null) { derivStakeRef.current = profile.stake; setDerivStake(profile.stake); }
    setDerivProposal(null);
    setRunImportStatus(`${profile.name} sélectionnée · appuyez sur Play`);
    setDerivAutoStatus("Stratégie prête · appuyez sur Play");
    return true;
  }

  async function importRunStrategyFile(event: React.ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;
    if (runStrategyLocked()) { setRunImportStatus("Import indisponible pendant une opération en cours."); return; }
    const importId = ++runImportIdRef.current;
    const contractType = derivContractTypeRef.current;
    try {
      if (file.size > 65536) throw new Error("Fichier trop volumineux (64 Ko maximum)");
      const profile = parseRunStrategyMarkdown(await file.text());
      if (importId !== runImportIdRef.current) return;
      if (runStrategyLocked() || derivContractTypeRef.current !== contractType) {
        setRunImportStatus("Import annulé : le contrat ou l’état du bot a changé."); return;
      }
      if (selectRunStrategy("imported", profile)) setImportedRunStrategy(profile);
    } catch (error) {
      if (importId === runImportIdRef.current) setRunImportStatus(`Import refusé · ${error instanceof Error ? error.message : "fichier invalide"}`);
    }
  }

  function changeDerivRunDuration(nextDuration: number) {
    if (!Number.isInteger(nextDuration) || nextDuration < 2 || nextDuration > 5
      || derivAutoRunningRef.current || derivOpenContractsRef.current.size
      || derivPendingBuysRef.current.size || derivAutoQuoteRef.current.size) return;
    runImportIdRef.current++;
    const next = { ...runStrategyRef.current, durationTicks: nextDuration };
    runStrategyRef.current = next;
    setRunStrategy(next);
    derivRunDurationRef.current = nextDuration;
    setDerivRunDuration(nextDuration);
    setDerivProposal(null);
  }
  ...
  function changeMatchDuration(durationTicks: number) {
    if (derivAutoRunningRef.current || derivOpenContractsRef.current.size || derivPendingBuysRef.current.size
      || derivAutoQuoteRef.current.size || derivContractTypeRef.current !== "DIGITMATCH"
  ...
  function changeDbxDigit(digit: number | null) {
    if (derivAutoRunningRef.current || derivOpenContractsRef.current.size || derivPendingBuysRef.current.size
      || derivAutoQuoteRef.current.size || derivMatchStrategyRef.current.executionMode !== "dbx_dynamic"
      || (digit !== null && (!Number.isInteger(digit) || digit < 0 || digit > 9))) return;
    const barrierMode: DerivAutoDigitBarrierMode = digit === null ? "dynamic" : "fixed";
    const next = { ...derivMatchStrategyRef.current, barrierMode, fixedDigit: digit };
    derivMatchStrategyRef.current = next;
    derivAutoDigitBarrierModeRef.current = barrierMode;
    setMatchStrategy(next);
    setDerivAutoDigitBarrierMode(barrierMode);
    if (digit !== null) {
      derivDigitBarrierRef.current = digit;
      setDerivDigitBarrier(digit);
    }
  ...
  function startDerivAuto() {
    ...
    const isAdvancedOverUnder = derivContractType === "DIGITOVER" || derivContractType === "DIGITUNDER";
	    ...
	    setDerivDoubleRiskSeriesIndex(0);
    setDerivAutoRunning(true);
    setDerivAutoStatus(derivContractType === "RUNHIGH" || derivContractType === "RUNLOW"
      ? evaluateRunEntry(derivTicksRef.current, derivContractType, runStrategyRef.current).reason
      : isAdvancedOverUnder
  ...
  function renderSecondaryView() {
    ...
    if (activeView === "copytrading") return <Mt5CopyTradingPanel onActive={setCopyTradingEnabled}/>;
    ...
    if (activeView === "derivbot") {
      ...
      const chartTicks = derivTicks.map((price, index) => ({ symbol: derivMarket, price, receivedAt: String(index) }));
	      ...
	      const isRunContract = derivContractType === "RUNHIGH" || derivContractType === "RUNLOW";
          const runDurationControl = <><label>Durée en ticks<select aria-label="Durée Only Ups / Only Downs en ticks" value={derivRunDuration} disabled={derivAutoRunning || derivDeals.some((deal) => deal.status === "open")} onChange={(event) => changeDerivRunDuration(Number(event.target.value))}>{[2, 3, 4, 5].map((duration) => <option key={duration} value={duration}>{duration} ticks</option>)}</select></label><p>Chaque tick doit conserver le sens choisi. Disponibilité et payout confirmés par la cotation Deriv.</p></>;
        ...
        <div className="view-heading"><div><p className="eyebrow">OPTIONS DIGITALES</p><h2>Terminal Deriv Bot</h2></div><div className="heading-actions"><Button className="secondary-action" onClick={() => navigate("derivhistory")}><WalletCards/> Historique des gains</Button><span className={`connection-chip ${derivConnected ? "online" : ""}`}><i/>{derivStatus === "real" ? "Compte réel connecté" : derivStatus === "demo" ? "Compte démo connecté" : derivStatus === "public" ? "Cours publics en direct" : "Connexion Deriv"}</span></div></div>
	        <div className="deriv-grid">
              ...
              {(Object.keys(derivContractCategories) as DerivContractCategory[]).map((category) => (
	            ...
	            {isRunContract && <RunStrategyPanel strategy={runStrategy} selection={runStrategySelection} imported={importedRunStrategy} disabled={derivAutoRunning || derivDeals.some((deal) => deal.status === "open")} status={runImportStatus} onSelect={selectRunStrategy} onImport={importRunStrategyFile}/> }
            {isPairedOverUnder && <OverUnderPairPanel mode={pairMode} rows={pairSessionMatches ? derivPairRows : []} trades={pairSessionMatches ? derivPairTrades : []} stats={pairSessionMatches ? derivPairStats : EMPTY_PAIR_STATS} stake={derivStake} currency={derivCurrency} running={derivAutoRunning} onStake={(value) => { derivStakeRef.current = value; setDerivStake(value); setDerivProposal(null); }}/>}
              ...
              {needsTouchBarrier(derivContractType) && <div className="digit-barrier"><label>Barrière cible<input type="text" value={derivTouchBarrier} onChange={(event) => { setDerivTouchBarrier(event.target.value); setDerivProposal(null); }} onBlur={() => setDerivTouchBarrier(normalizeTouchBarrier(derivTouchBarrier))} placeholder="+1"/></label><small>Utilisez une barrière relative, par exemple +1 ou -1.5.</small></div>}
	              <div className="ticket-fields"><label>{derivHalfBalanceRiskEnabled ? "Mise fallback (USD)" : "Mise (USD)"}<input type="number" min="0.35" step="0.01" disabled={derivHalfBalanceRiskEnabled} value={derivStake} onChange={(event) => { derivDoubleRiskSeriesIndexRef.current = 0; setDerivDoubleRiskSeriesIndex(0); setDerivStake(Number(event.target.value)); setDerivProposal(null); }}/></label><div className="auto-duration">{isRunContract ? runDurationControl : derivContractType === "DIGITMATCH" ? <><label>Durée manuelle en ticks<select aria-label="Durée Matches en ticks" value={matchStrategy.durationTicks ?? 1} disabled={derivAutoRunning} onChange={(event) => changeMatchDuration(Number(event.target.value))}>{Array.from({ length: 10 }, (_, i) => i + 1).map((duration) => <option key={duration} value={duration}>{duration} tick{duration > 1 ? "s" : ""}</option>)}</select></label><p>Durée choisie par vous, appliquée à chaque contrat. Le délai de cotation, d’achat et de règlement s’ajoute au temps observé. À modifier avant le lancement.</p></> : <><span>Durée choisie par le bot</span><b>{derivDuration} ticks <small>{derivDurationDecision.label}</small></b><p>{derivDurationDecision.reason}</p></>}</div></div>
              ...
              {isDbxGuarded && <div className="match-strategy-card">
                <b>Choix du digit DBX V3.1</b>
                <div className="ticket-fields">
                  <label>Mode du digit<select aria-label="Mode du digit DBX V3.1" value={matchStrategy.barrierMode} disabled={derivAutoRunning} onChange={(event) => changeDbxDigit(event.target.value === "dynamic" ? null : derivDigitBarrier)}><option value="dynamic">Adaptatif — choix du système</option><option value="fixed">Manuel — mon digit</option></select></label>
                  {matchStrategy.barrierMode === "fixed" && <label>Digit à matcher<select aria-label="Digit manuel DBX V3.1" value={matchStrategy.fixedDigit ?? 1} disabled={derivAutoRunning} onChange={(event) => changeDbxDigit(Number(event.target.value))}>{Array.from({ length: 10 }, (_, digit) => <option key={digit} value={digit}>{digit}</option>)}</select></label>}
                </div>
                <small>{matchStrategy.barrierMode === "fixed" ? `Le digit ${matchStrategy.fixedDigit} reste sélectionné. Son estimation doit passer le seuil configuré avant chaque achat.` : "Le système choisit parmi les deux digits les plus fréquents sur 50 ticks."}</small>
              </div>}
              {isDbxGuarded && <div className="match-strategy-card">
                ...
                <div className="ticket-fields">
                  ...
                  {matchStrategy.dbxMinimumProbability != null && <label>Estimation du modèle minimum (%)<input aria-label="Seuil manuel DBX V3.1 en pourcentage" type="number" min="0" max="100" step="0.1" disabled={derivAutoRunning} value={Number((matchStrategy.dbxMinimumProbability * 100).toFixed(4))} onChange={(event) => { if (event.target.value !== "") changeDbxThreshold(Number(event.target.value) / 100); }}/></label>}
                </div>
                <small>{matchStrategy.dbxMinimumProbability == null ? "Seuil automatique : équilibre du payout × 1,02 (marge de 2 % de la mise)." : "Le seuil manuel compare l’estimation du modèle, sans borne prudente ni marge de payout supplémentaire. Pendant la cotation, le score le plus faible est retenu. Une entrée peut passer sous l’équilibre du payout ; le budget de perte reste actif."}</small>
              </div>}
  ...
  return <main className="app-shell"><div className="noise"/>
    ...
    <section className="workspace">
      <header className="topbar"><button className="menu-mobile" onClick={() => setSidebar(true)} aria-label="Ouvrir le menu"><Menu/></button><div><p className="eyebrow">{viewTitles[activeView].eyebrow}</p><h1>{viewTitles[activeView].title}</h1></div>{activeView === "derivbot" || activeView === "derivhistory" ? <div className="top-actions"><div className={`system-pill ${derivStatus === "error" ? "danger" : ""}`}><span/><b>{derivStatus === "real" ? "DERIV RÉEL" : derivStatus === "demo" ? "DERIV DÉMO" : derivStatus === "public" ? "DERIV PUBLIC" : "CONNEXION DERIV"}</b></div><div className="avatar">PK</div></div> : <div className="top-actions"><div className={`system-pill ${stopped ? "danger" : ""}`}><span/><b>{activeView === "copytrading" ? (copyTradingEnabled ? "COPIE ACTIVE" : "COPIE EN PAUSE") : status}</b></div><Button className="emergency" onClick={() => activeView === "copytrading" ? window.dispatchEvent(new Event("copytrading-emergency-stop")) : setStopped(!stopped)}><Octagon/> {activeView === "copytrading" ? "Suspendre les nouvelles copies" : stopped ? "Réactiver" : "Arrêt d'urgence"}</Button><div className="avatar">PK</div></div>}</header>
      ...
      {activeView === "derivbot" && derivContractType !== "DIGITMATCH" && derivContractType !== "RUNHIGH" && derivContractType !== "RUNLOW" && <DigitPredictionBalloon
      ...
      {activeView === "derivbot" && derivContractType === "DIGITMATCH" && <MatchPredictionBalloon
        ...
        manualDigit={matchStrategy.executionMode === "dbx_dynamic" && matchStrategy.barrierMode === "fixed" ? matchStrategy.fixedDigit : null}
        selectionDisabled={matchStrategy.executionMode === "dbx_dynamic" && (derivAutoRunning || matchStrategy.barrierMode !== "fixed")}
        ...
        onSelectDigit={matchStrategy.executionMode === "dbx_dynamic" ? changeDbxDigit : changeDerivDigitBarrier}
      />}
    </section>
  </main>;
}
