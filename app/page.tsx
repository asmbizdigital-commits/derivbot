...
function parseAdvancedMatchStrategyMarkdown(markdown: string): ImportedMatchStrategy {
  ...
  const selectionMode = rulesData.selectionMode === "most_appearing_1000" || rulesData.selectionMode === "frequency_window" || rulesData.selectionMode === "top_two_frequency"
  ...
  return {
    ...
    barrierMode,
    fixedDigit,
    contractsPerSignal: selectionMode === "top_two_frequency" ? 1 : Math.trunc(clampNumber(data.contractsPerSignal, defaultImportedMatchStrategy.contractsPerSignal, 1, 10)),
...
function getDerivDigitAutoSignal(ticks: number[], contractType: DerivContractCode, preferredBarrier: number | null = null, pipSize = 3, overUnderStrategy: DerivOverUnderStrategy = "under8_transition", excludedMatchDigits: Set<number> = new Set(), matchRules: MatchStrategyRules = DEFAULT_MATCH_STRATEGY_RULES, completedMatchContracts = 0): DerivDigitAutoSignal | null {
  ...
  if (contractType === "DIGITMATCH") {
    ...
    const rankedCandidates = [...prediction.candidates]
      .filter((item) => fixedDigit !== null ? item.digit === fixedDigit : (matchRules.selectionMode === "top_two_frequency" || !excludedMatchDigits.has(item.digit)))
    ...
    return {
      ...
      confidence,
      reason: matchRules.selectionMode === "top_two_frequency"
...
export default function Home() {
  ...
  function maybeRunDerivAuto(currentTicks: number[], socket: WebSocket) {
	    ...
	    const priceSignal = currentContractType === "CALL" || currentContractType === "PUT" || needsTouchBarrier(currentContractType) ? buildRiseFallSignal(currentTicks, derivStrategyRef.current) : null;
		    if (currentContractType.startsWith("DIGIT") && !digitSignal) {
		      ...
		      if (currentContractType === "DIGITMATCH") {
		        ...
		        setDerivAutoStatus(currentMatchStrategy.rules.selectionMode === "top_two_frequency"
              ? `Most appearing 1er / 2e · ${Math.min(currentTicks.length, currentMatchStrategy.rules.minimumTicks)}/${currentMatchStrategy.rules.minimumTicks} ticks · attente classement`
    ...
    const positionTotal = currentContractType === "DIGITMATCH"
      ? currentMatchStrategy.rules.selectionMode === "top_two_frequency" ? 1 : Math.min(10, Math.max(1, Math.trunc(derivMatchPositionCountRef.current)))
    ...
    registerDerivSessionSignal();
    setDerivAutoStatus(`${signalReason}${currentMatchStrategy.rules.selectionMode === "top_two_frequency" && currentContractType === "DIGITMATCH" ? "" : ` · confiance ${signalConfidence}%`}${fixedBarrierSuffix}${martingaleSuffix} · ${positionTotal} position${positionTotal > 1 ? "s" : ""}`);
  ...
  async function importMatchStrategyFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setDerivAutoStatus(`${imported.name} · ${imported.rules.selectionMode === "top_two_frequency" ? `1er / 2e · fenêtre ${imported.rules.windowSize} ticks` : imported.rules.selectionMode === "frequency_window" ? `fenêtre ${imported.rules.windowSize} ticks` : "scan proba avancé"}`);
