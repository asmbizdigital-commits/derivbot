...
export function Mt5CopyTradingPanel({onActive}:{onActive:(active:boolean)=>void}) {
 ...
 async function mutate(input:Record<string,unknown>, urgent=false):Promise<boolean>{
  ...
  const task=actionQueue.current.then(async()=>{
   ...
   }catch(e){const message=e instanceof Error?e.message:"Action non confirmée";setError(message);if(input.action==="register"||input.action==="replace_master"||input.action==="update_master_server"||input.action==="archive_deleted_slave"||input.action==="settings")setFormError(message);return false;}
 ...
 return <section className="view-stack copy-live">
  ...
  {data&&<>
   ...
   <section className="panel"><div className="panel-head"><h3>Master : {master?.label??"à enregistrer"}</h3><b>{master?(master.online?"EN LIGNE":"HORS LIGNE"):"NON CONFIGURÉ"}</b></div>
    ...
    {master?.connectionError&&<p role="alert">Transmission MT5 refusée : {master.connectionError}</p>}
    ...
    {master?.canEditServer&&master.server===master.account&&<p role="status">Le serveur renseigné est identique au login MT5 ({master.account}). Vérifiez le nom du serveur dans MT5 et corrigez-le ci-dessous si nécessaire.</p>}
    {master?.canResetCredentials&&<p>Si MT5 affiche « Terminal non authentifié », recopiez les identifiants de ce master. Si vous ne les avez plus, régénérez-les ci-dessous : l’ancienne clé sera remplacée, le compte et les suiveurs seront conservés.</p>}
    <div className="copy-actions">{master?.canResetCredentials&&<button disabled={busy||!!issued} onClick={()=>void mutate({action:"reset_master_credentials",id:master.id})}>Régénérer les identifiants du master</button>}{master?.canEditServer&&<button disabled={busy} onClick={()=>{setReplacingMasterId(null);setEditingServerId(master.id);setForm({label:master.label,account:master.account,server:master.server===master.account?"":master.server,role:"master",mode:master.mode});setFormError("");setFormOpen(true);}}>Corriger le serveur MT5</button>}{master&&<button disabled={busy||!!issued} onClick={()=>{setEditingServerId(null);setReplacingMasterId(master.id);setForm({label:"Master réel",account:"",server:"",role:"master",mode:"real"});setFormError("");setFormOpen(true);}}>Changer de master</button>}{master&&<button disabled={busy||data.enabled||slaves.some(a=>a.managedCopies>0||a.pending)} onClick={()=>void mutate({action:"remove",id:master.id})}>Révoquer le master</button>}<button disabled={busy||!master?.online||data.enabled} onClick={()=>void mutate({action:"switch",enabled:true})}>Démarrer la copie</button><button disabled={busy||!data.enabled} onClick={()=>void mutate({action:"switch",enabled:false})}>Pause des nouvelles copies</button></div>
   ...
   <Dialog.Root open={formOpen} onOpenChange={open=>{if(!busy)setFormOpen(open);}}>
    <div className="copy-register-bar"><div><h3>Terminaux MT5</h3><p>{master?"Master enregistré":"Master à enregistrer"} · {slaves.length}/50 suiveurs</p></div><Dialog.Trigger asChild><button disabled={busy||!!issued||(!!master&&slaves.length>=50)} onClick={()=>{setEditingServerId(null);setReplacingMasterId(null);setForm({...form,role:master?"slave":"master",server:master?.server??form.server,label:"",account:""});setFormError("");}}>Ajouter un terminal</button></Dialog.Trigger></div>
    <Dialog.Portal><Dialog.Overlay className="copy-modal-overlay"/><Dialog.Content className="copy-live copy-modal" onPointerDownOutside={e=>e.preventDefault()} onEscapeKeyDown={e=>{if(busy)e.preventDefault();}}>
     <div className="copy-modal-heading"><div><Dialog.Title>{editingServerId?"Corriger le serveur MT5":replacingMasterId?"Changer de master":"Enregistrer un terminal"}</Dialog.Title><Dialog.Description>{editingServerId?"Saisissez le nom exact du serveur affiché dans MT5. Les AgentId et AgentKey de ce master restent identiques.":replacingMasterId?"Renseignez le nouveau compte master. Les slaves et leurs identifiants sont conservés. La copie sera mise en pause jusqu’à la connexion du nouveau master.":"Ajoutez un compte MT5 et générez ses identifiants de connexion."}</Dialog.Description></div><Dialog.Close asChild><button disabled={busy} aria-label="Fermer le formulaire">✕</button></Dialog.Close></div>
    ...
    const ok=await mutate(editingServerId?{action:"update_master_server",id:editingServerId,server:form.server}:{action:replacingMasterId?"replace_master":"register",...form,...(replacingMasterId?{id:replacingMasterId}:{})});if(ok){setEditingServerId(null);setReplacingMasterId(null);setFormOpen(false);setForm({...form,label:"",account:""});}
   }}>
    {!editingServerId&&<><label>Nom<input required value={form.label} maxLength={60} onChange={e=>setForm({...form,label:e.target.value})}/></label><label>Rôle<select disabled={!!replacingMasterId} value={form.role} onChange={e=>setForm({...form,role:e.target.value})}><option value="master" disabled={!!master&&!replacingMasterId}>Master</option><option value="slave" disabled={slaves.length>=50}>Suiveur ({slaves.length}/50)</option></select></label><label>Login MT5<input required inputMode="numeric" pattern="[0-9]+" value={form.account} onChange={e=>setForm({...form,account:e.target.value})}/></label><label>Compte<select value={form.mode} onChange={e=>setForm({...form,mode:e.target.value})}><option value="demo">Démo</option><option value="real">Réel</option></select></label></>}
    <label>Serveur MT5 exact<input required value={form.server} onChange={e=>setForm({...form,server:e.target.value})}/></label>
    {!editingServerId&&<p className="copy-map copy-volume-help">Aucun réglage de lot, plafond, inversion ou correspondance de symboles. La copie accepte des brokers et serveurs différents et transmet les valeurs exactes du master. L’exécution reste soumise à l’acceptation du broker.</p>}
    ...
    <button disabled={busy||!!(replacingMasterId&&data.masterReplacementError)}>{editingServerId?"Enregistrer le serveur":replacingMasterId?"Remplacer le master":"Créer les identifiants"}</button><button type="button" disabled={busy} onClick={()=>setFormOpen(false)}>Annuler</button>
