...
type Account = MonitorAccount & { id:string;label:string;role:"master"|"slave";account:string;server:string;mode:string;online:boolean;enabled:boolean;equity:number;positionCount:number;managedCopies:number;error:string;pending:{id:string;symbol:string;volume:number}|null;settings:{multiplier:number;maxLot:number;maxTotalLots:number;lossLimitPct:number;reverse:boolean;symbols:Record<string,string>} };
