...
export function Mt5CopyTradingPanel({onActive}:{onActive:(active:boolean)=>void}) {
 ...
 async function mutate(input:Record<string,unknown>, urgent=false):Promise<boolean>{
  ...
  const task=actionQueue.current.then(async()=>{
   ...
   }catch(e){const message=e instanceof Error?e.message:"Action non confirmée";setError(message);if(input.action==="register"||input.action==="replace_master"||input.action==="settings")setFormError(message);return false;}
