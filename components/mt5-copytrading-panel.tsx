 ...
 return <section className="view-stack copy-live">
  ...
  {data&&<>
   ...
   <section className="panel"><div className="panel-head"><h3>Master : {master?.label??"à enregistrer"}</h3><b>{master?(master.online?"EN LIGNE":"HORS LIGNE"):"NON CONFIGURÉ"}</b></div>
    ...
    <p>Copie identique 1:1 : toutes les positions ouvertes du master, mêmes symboles, BUY/SELL, lots et SL/TP. Même broker et même serveur requis. Les positions présentes au démarrage et à la reconnexion sont incluses.</p>
   ...
   <Dialog.Root open={formOpen} onOpenChange={open=>{if(!busy)setFormOpen(open);}}>
    <div className="copy-register-bar"><div><h3>Terminaux MT5</h3><p>{master?"Master enregistré":"Master à enregistrer"} · {slaves.length}/50 suiveurs</p></div><Dialog.Trigger asChild><button disabled={busy||!!issued||(!!master&&slaves.length>=50)} onClick={()=>{setForm({...form,role:master?"slave":"master",server:master?.server??form.server,label:"",account:""});setFormError("");}}>Ajouter un terminal</button></Dialog.Trigger></div>
    <Dialog.Portal><Dialog.Overlay className="copy-modal-overlay"/><Dialog.Content className="copy-live copy-modal" onPointerDownOutside={e=>e.preventDefault()} onEscapeKeyDown={e=>{if(busy)e.preventDefault();}}>
     <div className="copy-modal-heading"><div><Dialog.Title>Enregistrer un terminal</Dialog.Title><Dialog.Description>Ajoutez un compte MT5 et générez ses identifiants de connexion.</Dialog.Description></div><Dialog.Close asChild><button disabled={busy} aria-label="Fermer le formulaire">✕</button></Dialog.Close></div>
    ...
    const ok=await mutate({action:"register",...form});if(ok){setFormOpen(false);setForm({...form,label:"",account:""});}
   }}>
