...
export function OverUnderPairPanel({ rows, trades, stats, stake, currency, stopLoss, running, onStopLoss }: {
  ...
  return <section className="match-strategy-card pair-scanner" aria-label="Analyse multi-indices Under 5 et Over 4">
    ...
    <label>Budget de perte optionnel ({currency}, 0 = désactivé)<input type="number" min="0" step="0.1" value={stopLoss} disabled={running} onChange={(event) => onStopLoss(Number(event.target.value))}/></label>
    <small>{stopLoss === 0 ? "Budget monétaire désactivé : aucun plafond fixe de 2 par contrat. Les deux mises doivent tenir dans la balance disponible." : `Budget de perte actif : ${stopLoss.toFixed(2)} ${currency}.`}</small>
